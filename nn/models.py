"""
Basic graph encoder models: Encoder base class and ANN, GCN, GAT implementations.
No scvi-tools dependency; suitable for standalone use.
"""
import torch
...
class ANN(Encoder):
    def __init__(self, map, args, bias: bool = True):
        ...
        if self.args.num_classes is not None:
            ...
            if len(self.masks) == len(self.units):
...
class GAT(Encoder):
    def __init__(self, map, args, bias: bool = True, heads=1):
        ...
        for i in range(len(self.units) - 1):
            gat_conv = GATConv(self.units[i], self.units[i + 1], bias=self.bias, heads=heads)
            prune.custom_from_mask(gat_conv.lin, "weight", self.masks[i])
